...
import { sounds, vinylLoop } from "@/lib/sounds";
...
export function TrailControls() {
  ...
  return (
    <>
      <button
        data-no-trail
        onClick={() => setActive((v) => { const next = !v; if (next) { sounds.vinylStart(); vinylLoop.start(); } else { sounds.vinylStop(); vinylLoop.stop(); } return next; })}
